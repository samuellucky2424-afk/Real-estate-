...
const PropertyCard: React.FC<PropertyCardProps> = ({ property, isFavorite, onToggleFavorite, onSelectProperty }) => {
  ...
  const whatsAppMessage = encodeURIComponent(`Hello, I'm interested in the property: ${property.name}.`);
  const whatsAppLink = `https://wa.me/2348147133637?text=${whatsAppMessage}`;

  return (
    <div onClick={handleCardClick} className="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 group cursor-pointer flex flex-col">
      <div>
      ...
      <div className="p-6 pt-2 mt-auto">
        <div className="border-t pt-4 flex justify-between items-center text-sm font-semibold">
           <a href={whatsAppLink} onClick={handleContactClick} target="_blank" rel="noopener noreferrer" className="text-brand-blue hover:underline z-20 relative">
             Contact Agent
           </a>
